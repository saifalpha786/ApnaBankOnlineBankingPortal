...
import { WeatherNewsContext } from "./WeatherNewsContext";
import WeatherNewsArticle from "./WeatherNewsArticle";

function WeatherNews(props) {
  const { data } = useContext(WeatherNewsContext);
  console.log(data);
      ...
      <div style={{ marginTop: '10px' }}>
        {visibleData.length > 0 ? (
          visibleData.map((news) => (
            <WeatherNewsArticle data={news} key={news.link} />
          ))
        ) : (
          <p>Loading...</p>
        )}
